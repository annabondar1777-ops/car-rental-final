...
export const handler = async (event) => {
  try{
    ...
    const path   = 'data/settings.json'; if(!token||!repo) return { statusCode:500, body: JSON.stringify({error:'Missing env: GITHUB_TOKEN or REPO_FULL'}) };
    const api='https://api.github.com';
    const getUrl = `${api}/repos/${repo}/contents/${encodeURIComponent(path)}?ref=${encodeURIComponent(branch)}`;
    let sha; const getRes = await fetch(getUrl, { headers:{ Authorization:`token ${token}`, 'User-Agent':'netlify-func' } });
    if(getRes.ok){ const j=await getRes.json(); sha=j.sha; }
    const content = Buffer.from(JSON.stringify(s, null, 2)).toString('base64');
    const putRes = await fetch(`${api}/repos/${repo}/contents/${encodeURIComponent(path)}`, {
      ...
      body: JSON.stringify({ message:'Update settings.json via Netlify', content, branch, sha })
    });
